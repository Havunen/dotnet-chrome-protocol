...
public class ProtocolSchemaValidator
{
  ...
  public async Task<(bool, IEnumerable<string>)> TryValidateAsync(string fileName, JsonDocument protocolDocument)
  {
    ...
    var result = schema.Evaluate(protocolDocument);
    if (!result.IsValid)
    {
      var presentableErrors = FlattenResults(result)
        .Where(results => results.Errors != null)
  ...
  {
    static void CollectResults(EvaluationResults node, LinkedList<EvaluationResults> acc)
    {
      acc.AddLast(node);
      if (node.HasDetails)
      {
